...
ROUTES={
 # ── 2막 ──
 33:[("era","시대:붕괴기",1,0.50),("seek","탐구",1,0.72)],          # 벽에 이름을 적던 병사
 35:[("guard","수호",2,0.70),("devote","헌신",1,0.74)],
 38:[("seek","탐구",2,0.62),("era","시대:붕괴기",1,0.55)],          # 표식을 세는 일
 ...
 55:[("seek","탐구",2,0.60),("era","시대:균열기",1,0.58)],          # 이어 붙인 자리
 ...
 24:[("era","시대:붕괴기",1,0.45),("guard","수호",1,0.60),("trade","생업:담장",1,0.55)],
 25:[("flee","도피",1,0.70)],
 28:[("guard","수호",2,0.65),("era","시대:붕괴기",1,0.60)],
...
for f in range(1,61):
    ...
    for rid,cond,cnt,mult in ROUTES.get(f,[]):
        if ":" in cond: ctype,cval = cond.split(":",1)
