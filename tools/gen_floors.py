...
ERA=lambda f:"종말기" if f<=20 else "붕괴기"
...
ROUTES={
 ...
 14:[("devote","헌신",1,0.75)],
 15:[("seek","탐구",1,0.75),("guard","수호",2,0.70)],
 20:[("guard","수호",2,0.72),("seek","탐구",1,0.75),("weak","약체",3,0.65)],
 24:[("era","시대:24",1,0.45),("guard","수호",2,0.60),("trade","생업:담장",1,0.55)],
 25:[("flee","도피",1,0.70)],
 28:[("guard","수호",2,0.65),("era","시대:24",1,0.60)],
...
for f in range(1,31):
    ...
    for rid,cond,cnt,mult in ROUTES.get(f,[]):
        routes.append({"id":rid,"power":mult,
                       "conditions":[{"type":cond.split(":")[0] if ":" in cond else "역할",
                                      "value":cond.split(":")[1] if ":" in cond else cond,
                                      "count":cnt}]})
    floors.append({"floor":f,"world":1,"era":ERA(f),"name":NAMES[f],
                   "teams":t,"slot":slot(f),"members_required":need,
                   "base_power":req,"routes":routes})

json.dump({"world":1,"act":1,"floors":floors},
...
print()
print("보유 잔상 수 추이:", ", ".join(f"{f}층={len(roster_at(f))}" for f in (5,10,15,20,25,30)))
